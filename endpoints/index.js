...
async function askCerebras(model, prompt) {
  const response = await axios.post(
    ...
    { model, messages: [{ role: "user", content: prompt }], max_completion_tokens: 8192, temperature: 0.7, top_p: 0.8 },
...
async function askAIML(model, prompt) {
  const response = await axios.post(
    ...
    { model, messages: [{ role: "user", content: prompt }], temperature: 1.9, top_p: 1, frequency_penalty: 1.9, max_tokens: 1536, top_k: 100 },
